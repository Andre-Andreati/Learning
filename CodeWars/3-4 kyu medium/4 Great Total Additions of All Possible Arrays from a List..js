function gta(limit, ...args){
    ...
    for (let i=0; i<limit; i++) {
      for (let j=0; j<args.length; j++) {
        let elem = args[j].toString()[i];
        if (elem >= 0 && !baseList.includes(elem)) baseList.push(elem)
      }
    }
    baseList = baseList.slice(0,limit);
    ...
    let nextArray = function(arr1, arr2) {
      let ans = [];
    ...
    arrays[1] = baseList;
    for (let i=0; i<baseList.length; i++){
      arrays[i+2] = nextArray(arrays[i+1], baseList)
    }
    return sum;
  }
